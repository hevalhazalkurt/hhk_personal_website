...
(function() {
  function m(a, g) {
    var b = a.editable().findOne('a[data-cke-autoembed="' + g + '"]'),
      c = a.lang.autoembed,
      d;
    if (b && b.data("cke-saved-href")) {
      ...
      if (e) {
        var f = "function" == typeof e.defaults ? e.defaults() : e.defaults,
          ...
          h, k = a.widgets.wrapElement(f, e.name),
          l = new CKEDITOR.dom.documentFragment(k.getDocument());
        l.append(k);
        (h = a.widgets.initOn(f, e)) ? (d = a.showNotification(c.embeddingInProgress,
          ...
          callback: function() {
            ...
            if (b) {
              ...
              var j = c.createBookmarks(!1)[0],
                i = j.startNode,
                h = j.endNode || i;
              CKEDITOR.env.ie && (9 > CKEDITOR.env.version && !j.endNode && i.equals(b.getNext())) && b.append(i);
              e.setStartBefore(b);
              e.setEndAfter(b);
              f.insertElement(k, e);
              f.contains(i) && f.contains(h) ? c.selectBookmarks([j]) :
                (i.remove(), h.remove());
              a.fire("unlockSnapshot")
            }
            d.hide();
            a.widgets.finalizeCreation(l)
          },
      ...
      } else window.console && window.console.log("[CKEDITOR.plugins.autoEmbed] Incorrect config.autoEmbed_widget value. No widget definition found.")
    }
  }
  var n = /^<a[^>]+href="([^"]+)"[^>]*>([^<]+)<\/a>$/i;
  CKEDITOR.plugins.add("autoembed", {
    requires: "autolink,undo",
    lang: "cs,de,en,it,ku,nb,pl,pt-br,tr,zh",
    init: function(a) {
      var g = 1,
        b;
      a.on("paste", function(c) {
        if (c.data.dataTransfer.getTransferType(a) == CKEDITOR.DATA_TRANSFER_INTERNAL) b = 0;
          ...
          var d = c.data.dataValue.match(n);
          if (b = null != d && decodeURI(d[1]) == decodeURI(d[2])) c.data.dataValue = '<a data-cke-autoembed="' + ++g + '"' + c.data.dataValue.substr(2)
        }
      }, null, null, 20);
